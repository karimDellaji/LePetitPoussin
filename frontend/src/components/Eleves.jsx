...
export default function Eleves() {
  ...
  // CORRIGÉ: Utilise 'estPaye' au lieu de 'paye'
  ...
  const supprimerEleve = async (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer cet élève ?")) {
      await fetch(`${API_BASE_URL}/api/children/${id}`, { method: 'DELETE' });
      fetchEleves();
    }
  };
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
        <h2 className="text-lg font-black mb-6 flex items-center gap-2 uppercase tracking-tighter">
          <UserPlus className="text-emerald-500" size={20}/> Inscription
        </h2>
        <form onSubmit={ajouterEleve} className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <input 
            className="bg-gray-50 p-4 rounded-2xl border-none outline-none text-sm" 
            placeholder="Nom" 
            value={form.nom} 
          ...
          />
          <select 
            className="bg-gray-50 p-4 rounded-2xl border-none font-bold text-xs" 
            value={form.section} 
          ...
          <button 
            type="submit" 
            className="md:col-span-5 bg-gray-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] hover:bg-emerald-500 transition-all shadow-xl shadow-emerald-100"
          >
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/20">
          <div className="relative w-64">
            ...
            <input 
              className="pl-10 pr-4 py-2 bg-white rounded-xl border-none text-[10px] w-full shadow-inner" 
              ...
              {eleves
                ...
                <tr key={el._id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="p-6">
                    ...
                    <p className="text-[10px] font-bold text-emerald-500">ID: {el.parentCode}</p>
                  </td>
                  ...
                  <td className="p-6">
                    <button 
                      ...
                      className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[9px] font-black uppercase transition-all active:scale-90 ${
                        el.estPaye ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200' : 'bg-red-500 text-white shadow-lg shadow-red-200'
                      }`}
                    >
                      {el.estPaye ? <CheckCircle size={14}/> : <XCircle size={14}/>}
                      {el.estPaye ? "Payé" : "Non Payé"}
                    </button>
                  </td>
