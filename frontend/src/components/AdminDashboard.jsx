    ...
    <div className="flex min-h-screen bg-[#FDFEFE] font-sans">
      ...
      <main className="ml-72 flex-1 p-12">
        <header className="flex justify-between items-center mb-12">
          ...
          {activeTab !== 'dashboard' && (
            <button 
              onClick={() => { 
                setModalType(activeTab === 'finances' ? 'finance' : activeTab); 
                setShowModal(true); 
              }} 
            ...
            >
              <Plus size={18}/> 
              Ajouter {activeTab === 'finances' ? 'Transaction' : 'Nouveau'}
            </button>
          )}
        </header>
        ...
        {loading && (
          <div className="text-center py-20">
            <p className="text-slate-400 font-bold uppercase text-xs tracking-widest">Chargement...</p>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              {filteredChildren.map(c => (
                <div 
                  key={c._id} 
                  className="bg-white p-8 rounded-[2.5rem] shadow-2xl shadow-slate-100 relative group"
                >
                  <button 
                    onClick={() => deleteItem('children', c._id)} 
                  ...
                  <div className="border-t pt-4 flex justify-between items-center">
                    <button 
                      onClick={() => togglePayment(c._id, c.estPaye)}
                      className={`text-[10px] font-black uppercase flex items-center gap-2 transition-colors ${
                        c.estPaye ? 'text-emerald-500' : 'text-red-400'
                      }`}
                    >
                      {c.estPaye ? <CheckCircle2 size={14}/> : <XCircle size={14}/>} 
                      {c.estPaye ? 'Payé' : 'Impayé'}
                    </button>
                    <span className="bg-slate-900 text-white text-[9px] px-3 py-1 rounded-full">
                      {c.parentCode}
                    </span>
                  </div>
                </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {staff.map(s => (
                  <tr key={s._id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-8 font-bold text-slate-400 text-xs">{s.telephone}</td>
                    <td className="p-8 font-mono font-black text-emerald-500">{s.loginCode}</td>
                    <td className="p-8">
                      <button 
                        onClick={() => deleteItem('staff', s._id)} 
            ...
            <div className="bg-white rounded-[3rem] shadow-2xl p-10">
              ...
              <div className="space-y-4">
                {finances.transactions.map(t => (
                  <div 
                    key={t._id} 
                    className="flex justify-between items-center p-6 bg-slate-50 rounded-2xl group"
                  >
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl ${
                        t.type === 'Recette' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'
                      }`}>
                    ...
                    <div className="flex items-center gap-6">
                      <p className={`font-black text-lg ${
                        t.type === 'Recette' ? 'text-emerald-500' : 'text-red-500'
                      }`}>
                        {t.type === 'Recette' ? '+' : '-'}{t.montant} DT
                      </p>
                      <button 
                        onClick={() => deleteItem('finance', t._id)} 
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white w-full max-w-lg rounded-[3rem] p-12 relative max-h-[90vh] overflow-y-auto">
            <button 
            ...
            <h2 className="text-2xl font-black text-slate-800 uppercase mb-8">
              Nouveau {modalType}
            </h2>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              {modalType === 'eleves' && (
                <>
                  ...
                  />
                  <select 
                    className="w-full bg-slate-50 p-5 rounded-2xl font-bold" 
                  ...
                  <Input 
                    placeholder="Tarif" 
                    type="number" 
                    onChange={v => setForm({...form, tarif: Number(v)})} 
                  />
                  <Input 
                    placeholder="Tél Parent" 
                    onChange={v => setForm({...form, parentTel: v})} 
                  />
                </>
              )}
              ...
              {modalType === 'staff' && (
                <>
                  <Input 
                    ...
                    onChange={v => setForm({...form, nomComplet: v})} 
                  />
                  <select 
                    className="w-full bg-slate-50 p-5 rounded-2xl font-bold" 
                  ...
                  <Input 
                    placeholder="Salaire" 
                    type="number" 
                    onChange={v => setForm({...form, salaire: Number(v)})} 
                  />
                  <Input 
              ...
              {modalType === 'finance' && (
                <>
                  ...
                  <Input 
                    placeholder="Montant" 
                    type="number" 
                    onChange={v => setForm({...form, montant: Number(v)})} 
                  />
                  <Input 
              ...
              <button 
                type="submit"
                className="w-full bg-emerald-500 text-white font-black py-5 rounded-2xl uppercase text-xs tracking-widest mt-4 shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition-colors"
              >
                Enregistrer
              </button>
            </form>
...
const MenuBtn = ({ icon, label, active, onClick }) => (
  <button 
    onClick={onClick} 
    className={`w-full flex items-center gap-4 p-5 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all ${
      active ? 'bg-emerald-500 text-white shadow-xl shadow-emerald-100' : 'text-slate-400 hover:bg-slate-50'
    }`}
  >
    {icon} {label}
  </button>
...
const StatCard = ({ label, value, icon }) => (
  <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-50">
    <div className="flex justify-between items-start mb-6">
      ...
      <p className="font-black text-[10px] uppercase text-slate-300">{label}</p>
    </div>
...
const Input = ({ placeholder, type = "text", onChange }) => (
  <input 
    ...
    className="w-full bg-slate-50 p-5 rounded-2xl outline-none font-bold text-slate-700 border border-transparent focus:border-emerald-200" 
