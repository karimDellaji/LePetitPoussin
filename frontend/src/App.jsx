...
export default function App() {
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFB] flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl p-12 border border-emerald-50 text-center">
          ...
          <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mb-10">Jardin d'enfants Fresh Emerald</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            {mode === 'code' ? (
              <div className="relative">
                ...
                <input 
                  className="w-full bg-slate-50 p-5 pl-14 rounded-2xl outline-none font-bold text-slate-700 border border-transparent focus:border-emerald-200"
              ...
              <>
                <div className="relative">
                  ...
                  <input 
                    className="w-full bg-slate-50 p-5 pl-14 rounded-2xl outline-none font-bold focus:border-emerald-200 border border-transparent"
                ...
                <input 
                  type="password"
                  className="w-full bg-slate-50 p-5 rounded-2xl outline-none font-bold focus:border-emerald-200 border border-transparent"
            ...
            <button type="submit" className="w-full bg-emerald-500 text-white font-black py-5 rounded-2xl uppercase text-xs tracking-widest shadow-lg shadow-emerald-100 hover:bg-slate-800 transition-all">
              Se Connecter
            </button>
          </form>
  ...
  return (
    <>
      {role === 'admin' && <AdminDashboard onLogout={() => setUser(null)} />}
      {role === 'teacher' && <TeacherDashboard teacher={user} onLogout={() => setUser(null)} />}
      {role === 'parent' && <ParentDashboard child={user} onLogout={() => setUser(null)} />}
    </>
  );
}